...
export function showGeneratorHelp(toolbox: GluegunToolbox) {
  ...
  if (inIgnite) {
    ...
    warning(
      '⚠️  Not in an Ignite project root. Go to your Ignite project root to see generators.',
    );
  }
}
...
export function updateGenerators(toolbox: GluegunToolbox) {
  ...
  const distinct = (val: any, index: any, self: any) =>
...
export function generateFromTemplate(
  ...
  const newFiles = files.map((templateFilename: string) => {
    ...
    if (
      ...
      const exportExists = indexContents && indexContents.includes(exportLine);
...
export function installGenerators(generators: string[]): string[] {
  ...
  if (!isIgniteProject()) {
    throw new Error(
      "Not in an Ignite root directory (can't find ./ignite folder)",
    );
  }
